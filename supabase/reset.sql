...
-- ─── 1. Drop old objects ───
drop function if exists sync_payment_bank_deduction(uuid, boolean);
drop table if exists bank_fund_outflows cascade;
...
drop table if exists payment_makers cascade;
drop table if exists payment_sources cascade;
drop table if exists bank_funds cascade;
...
create table bank_fund_outflows (
  id uuid primary key default gen_random_uuid(),
  wedding_id uuid not null references weddings(id) on delete cascade,
